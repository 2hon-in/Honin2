        ...
        ()=>{
            ...
            .then((result=>{
                if( result.data.error == 'ERROR_LOGIN' ){
                    ...
                    // .then((res)=>{   dispatch( setFollowings({ followings:res.data} ) );  })
                    // jaxios.get('/api/member/getFollowers',  { params : { nickname:result.data.nickname }} )
                    // .then((res)=>{   dispatch( setFollowers({ followers:res.data} ) );   })
                    navigate('/')
                }
            }))
